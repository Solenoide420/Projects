...
## A faire
# Initialiser des orbites avec des satellites dessus (repartis également ? -> début OUI)
# Faire la verif en se servant de ça (si répartis également : changer la disposition des autres)
...
def Main(INIT_SAT, NB_ORBITES):
    orbites = init_orbites(INIT_SAT, NB_ORBITES)
    points = init_points() if TERRE_ENTIERE else init_points2()
    BEST = orbites
    actuel = best = INIT_SAT
    tests = 0
    config = 0
